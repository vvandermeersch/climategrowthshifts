...
generated quantities {
  ...
  for (t in 1:N_trees) {
    ...
    //mu1[tree_idxs] = alpha + beta_gsl * (gsl_obs_tree - gsl0) + beta_gdd * (gdd_obs_tree - gdd0) 
    //+ beta_sm * (sm_obs_tree - sm0);
    // mu1[tree_idxs] = alpha + f_sh[stand_idx, all_years_idxs_tree];
    mu1[tree_idxs] = alpha + beta_gsl * (gsl_obs_tree - gsl0) + beta_gdd * (gdd_obs_tree - gdd0) 
    + beta_sm * (sm_obs_tree - sm0) + f_sh[stand_idx, all_years_idxs_tree];
    ...
    mu3[tree_idxs] = gp_pred_rng(years_tree, log_rw_obs[tree_idxs], years_tree, 
    mu2[tree_idxs], gamma_sh, rho_sh, sigma, 1e-10) - mu2[tree_idxs]; // and kappa?
    
    //fsum[tree_idxs] = mu1[tree_idxs] + mu2[tree_idxs];                          
    fsum[tree_idxs] = mu1[tree_idxs] + mu2[tree_idxs] + mu3[tree_idxs];
    ...
    f[tree_idxs] = gp2_pred_rng(years_tree, log_rw_obs[tree_idxs], years_tree,
    mu1[tree_idxs], gamma_sh, rho_sh, gamma[species_idx], rho[species_idx], sigma, 1e-10); // and kappa?
                                
    mu[tree_idxs] =  alpha
    + beta_gsl * (gsl_obs_tree - gsl0)
    + beta_gdd * (gdd_obs_tree - gdd0)
    + beta_sm * (sm_obs_tree - sm0)
    + f_sh[stand_idx, all_years_idxs_tree]; // kappa omitted
    
    log_rw[tree_idxs] = gp_pred_rng(years_tree, log_rw_obs[tree_idxs], years_tree, 
    mu[tree_idxs], gamma[species_idx], rho[species_idx], sigma, 1e-10);
