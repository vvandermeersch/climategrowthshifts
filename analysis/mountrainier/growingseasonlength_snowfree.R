...
ids_missing <- which(is.na(snow[, 'snowdepth']) & snow[, 'year'] == 1970 & 
                       format(snow$date, '%m-%d') >=  format(as.Date('03-15', '%m-%d'), '%m-%d'))
...
for(y in as.character(1931:2010)){
  ...
  df$snowfreecount <- NA
  snowfreecount <- zoo::rollapply(df$snowdepth == 0, 7, sum, na.rm = TRUE, align = 'left')
  df[1:length(snowfreecount), 'snowfreecount'] <- snowfreecount
  snowfreedays <- df[df$date >=  as.Date(paste0(y, '-03-15')) & df$snowfreecount == 7,]
