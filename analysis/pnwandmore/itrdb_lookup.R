...
filenames <- filenames[grepl(filenames, pattern = '*-rwl-noaa.txt$')]
...
filenames <- filenames[grepl(filenames, pattern = '*-rwl-noaa.txt$')]
datasets <- sub("-.*", "", filenames)
ringwidth_series <- readRDS(file.path(wd, 'input', 'itrdb', 'ringwidth_series_canadaonly.rds'))
datasets_filtered <- unique(ringwidth_series$dataset)
data_summary <- data.frame()
for(f in filenames[which(datasets %in% datasets_filtered)]){
...
saveRDS(data_summary, file = file.path(wd, 'input', 'itrdb', 'datasets_summary_canadaonly.rds'))

server <- "https://www.ncei.noaa.gov/pub/data/paleo/treering/measurements/northamerica/mexico"
filenames <- unlist(xml2::as_list(httr::content(httr::GET(server))))
filenames <- filenames[grepl(filenames, pattern = '*-rwl-noaa.txt$')]
datasets <- sub("-.*", "", filenames)
ringwidth_series <- readRDS(file.path(wd, 'input', 'itrdb', 'ringwidth_series_mexicoonly.rds'))
datasets_filtered <- unique(ringwidth_series$dataset)
data_summary <- data.frame()
for(f in filenames[which(datasets %in% datasets_filtered)]){
...
saveRDS(data_summary, file = file.path(wd, 'input', 'itrdb', 'datasets_summary_mexicoonly.rds'))


data_summary_all <- rbind(readRDS(file = file.path(wd, 'input', 'itrdb', 'datasets_summary_usonly.rds')),
                          readRDS(file = file.path(wd, 'input', 'itrdb', 'datasets_summary_canadaonly.rds')),
                          readRDS(file = file.path(wd, 'input', 'itrdb', 'datasets_summary_mexicoonly.rds')))
saveRDS(data_summary_all, file = file.path(wd, 'input', 'itrdb', 'datasets_summary_all.rds'))
