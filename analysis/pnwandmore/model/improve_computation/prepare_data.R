...
stands_sequence <- sapply(seq(5,95,10), function(x) sample(unique(datasets$grouped_stand), x))

datasets_all <- datasets

for(stands_tokeep in stands_sequence){
  
  datasets <- datasets_all[datasets_all$grouped_stand %in% stands_tokeep,]
  ...
  saveRDS(data, file = file.path(wd, 'model/improve_computation/output/model', paste0("data_05dec2025_PIPO_",length(stands_tokeep),"stands.rds")))
  saveRDS(datasets, file.path(wd, 'model/improve_computation/output/model', paste0("datasets_05dec2025_PIPO_",length(stands_tokeep),"stands.rds")))
