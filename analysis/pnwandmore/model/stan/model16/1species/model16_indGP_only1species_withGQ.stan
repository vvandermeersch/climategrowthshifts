            ...
            profile("store_trees") {
              for(y in 1:N_years[t]) {
                ...
                if(rw_obs[idx] >= epsilon){
                  ...
                  log_p0[ys] += log_mix(omega_nonconc_sck[s],
...
parameters {
  ...
  // Probability of tree-level shock given stand NOT in shock (non-concordant shock)
  real mu_logdelta_omega_nonconc_sck;
  real<lower=0> tau_logdelta_omega_nonconc_sck;
  vector[N_stands] logdelta_omega_nonconc_sck;
...
model {
  ...
  rho_sp ~ lognormal(3.55, 0.24); // 20 <~ rho <~ 60 // MODIFIED THIS
...
generated quantities {
  ...
  vector[N] delta_nonconc_sck = rep_vector(0,N); // latent amplitude of concordant shock
  ...
  for (t in 1:N_trees) {
    ...
    f[tree_start:tree_end] = L_cov[1:N_years[t], 1:N_years[t]] * f_tilde[tree_start:tree_end]; 
    ...
    real conc_shock = phi_sck[stand_idx]*omega_conc_sck[stand_idx];
    real nonconc_shock = (1-phi_sck[stand_idx])*omega_nonconc_sck[stand_idx];
    ...
    for(y in 1:N_years[t]){
      ...
      if(sck_cat == 3){
        ...
        if(sck_cat == 1){
          ...
          log_rw_pred[idx] = normal_rng(mu_f + delta_conc_sck[idx], sigma);
        }else{
          delta_nonconc_sck[idx] = normal_rng(conjugate_mean, conjugate_sd);
          log_rw_pred[idx] = normal_rng(mu_f + delta_nonconc_sck[idx], sigma);
        }
        
      }else{
        // sample from a truncated normal distribution? between -inf and log(epsilon)
        real log_y = normal_ub_rng(mu_f, sqrt(tau_sck^2 + sigma^2), log(epsilon));
