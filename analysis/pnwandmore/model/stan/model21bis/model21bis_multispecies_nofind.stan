        ...
        for (y in 1:N_years[t]) {
          ...
          if (rw_obs[idx] >= epsilon){
            ...
            vector[4] lpds = [
              ...
              normal_lpdf(log_rw | mu_f, sqrt(tau_conc[sp]^2 + sigma^2)), // concordant depressed growth, no idio. shutdown
              normal_lpdf(log_rw | mu_f, sqrt(tau_idio^2 + tau_conc[sp]^2 + sigma^2)) // concordant depressed growth, idio. shock
            ]';
            ...
            vector[9] lpds = [
              ...
              normal_lcdf(log(epsilon)| mu_f, sqrt(tau_conc[sp]^2 + sigma^2)), // concordant depressed growth, no idio. shutdown
              normal_lcdf(log(epsilon) | mu_f, sqrt(tau_idio^2 + tau_conc[sp]^2 + sigma^2)), // concordant depressed growth, idio. shock
...
transformed parameters {
  ...
  for (sp in 1:N_species) {
    matrix[N_all_years, N_all_years] cov_ind
    =  gp_exp_quad_cov(all_years, gamma_merged[sp], rho_merged[sp])
    + diag_matrix(rep_vector(1e-8, N_all_years));
