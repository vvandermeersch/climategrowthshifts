...
data {
  ...
  int<lower=1> N_standsxspecies;    // Number of stands x species
  ...
  // Indices of tree speciesxstands
  array[N_trees] int<lower=1, upper=N_standsxspecies> standxspecies_idxs;
  
  // Indices of stands species
  // array[N_stands] int<lower=1, upper=N_species> standxspecies_species_idxs;
  ...
  array[N_standsxspecies] vector<lower=0, upper=1>[N_all_years] w_sck;
...
parameters {
  ...
  // Growth shocks
  array[N_standsxspecies] vector[N_all_years] delta_tilde_sck; // Latent parameter for yearly shocks
  ...
  // vector<lower=0, upper=1>[N_species] gamma_sck; // Inner core probability
...
transformed parameters {
  ...
  array[N_standsxspecies] vector[N_all_years] delta_sck;
  for (s in 1:N_standsxspecies) {
...
model {
  ...
  // Shocks!
  for (s in 1:N_standsxspecies) {
    // sp = standxspecies_species_idxs[s];
    for(y in 1:N_all_years) {
  ...
  for (t in 1:N_trees) {
    ...
    int standxspecies_idx = standxspecies_idxs[t];
    ...
    + delta_sck[standxspecies_idx,all_years_idxs_tree];
