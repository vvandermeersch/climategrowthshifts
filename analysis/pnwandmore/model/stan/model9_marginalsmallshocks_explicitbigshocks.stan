...
parameters {
  ...
  // Growth shocks
  vector[N] delta_sck; // Latent parameter for yearly shocks
...
model {
  ...
  delta_sck ~ normal(0, outer_tau_sck);
    ...
    for(ts in 1:N_stand_trees[s]){
      ...
      for(y in 1:N_years[t]) {
        ...
        log_p0[ys] += log_mix(omega_nonconc_sck,
                          normal_lpdf(log_rw_obs[tree_idxs[y]] | mu[tree_idxs[y]] + f[tree_idxs[y]] + delta_sck[tree_idxs[y]], sigma),
