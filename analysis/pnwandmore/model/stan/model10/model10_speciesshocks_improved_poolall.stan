...
transformed parameters {
  ...
  for (s in 1:N_stands) {
    phi_sck[s] = inv_logit(alpha_phi_sck[s]);
  }
  
  // Probability of tree-level shock given stand in shock (concordant shock)
  vector<lower=0, upper=1>[N_stand_species] omega_conc_sck; 
  for (s in 1:N_stand_species) {
    omega_conc_sck[s] = inv_logit(alpha_omega_conc_sck[s]);
  }
...
model {
  ...
  for (s in 1:N_stands){
    ...
    alpha_phi_sck[s] ~ normal(logit(phi_sck0), tau_phi_sck);
  }
  
  for (s in 1:N_stand_species){
    alpha_omega_conc_sck[s] ~ normal(logit(omega_conc_sck0), tau_omega_conc_sck);
  }
