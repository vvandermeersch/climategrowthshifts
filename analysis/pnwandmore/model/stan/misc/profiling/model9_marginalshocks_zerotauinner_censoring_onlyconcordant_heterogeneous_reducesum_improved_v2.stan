  ...
  real loglikelihood_partial_sum(array[] int stand_ids_slice,
    ...
    for (i in 1:(end-start+1)) {
      ...
      for(ts in 1:N_stand_trees[s]){
        ...
        int tstart = tree_start_idxs[t];
        int tend = tree_end_idxs[t];
        int species_idx = species_idxs[t];
        
        array[N_years[t]] int tree_idxs = linspaced_int_array(N_years[t], tstart, tend);
        array[N_years[t]] int all_years_idxs_tree = all_years_idxs[tree_idxs];
        
        vector[N_years[t]] f = block(L_cov[species_idx], 1, 1, N_years[t], N_years[t]) * f_tilde[tree_idxs];
        ...
        for(y in 1:N_years[t]) {
          ...
          if(rw_obs[tree_idxs[y]] >= epsilon){
            ...
            log_p1[ys] += log_mix(omega_conc_sck[s],
                            ...
                            + f[y], sqrt(outer_tau_sck^2 + sigma^2)),
                            normal_lpdf(log(rw_obs[tree_idxs[y]]) | mu[tree_idxs[y]] 
                            + f[y], sigma));
          }else{
...
parameters {
  ...
  vector<lower=0, upper=1>[N_stands] omega_conc_sck; // Probability of tree-level shock given stand in shock (concordant shock)
...
model {
  ...
  profile("prepare") {
    for (t in 1:N_trees) {
      ...
      + beta_sm[species_idx] * (sm_obs[start:end] - sm0)
      + beta_vpd[species_idx] * (vpd_obs[start:end] - vpd0)
      + f_sh[stand_idx, all_years_idxs[start:end]];
...
//     real mw_shock = phi_sck[stand_idx]*omega_conc_sck[stand_idx];
//     real log_pshock;
...
//           normal_lpdf(log(rw_obs[tree_idxs[y]])| mu[y] + f[tree_idxs[y]], sqrt(outer_tau_sck^2 + sigma^2)),
...
//           sqrt(outer_tau_sck^2 + sigma^2));
//         log_pshock_plus_pnonshock = log_mix(mw_shock,
//           normal_lcdf(log(epsilon)| mu[y] + f[tree_idxs[y]], sqrt(outer_tau_sck^2 + sigma^2)),
