...
parameters {
  ...
  real<lower=1, upper = rho_sp> rho_sh;   // Time scale
...
model {
  ...
  beta_ffp ~ normal(0, log(1.8) / 2.57); // -log(1.8) <~ beta_gsl <~ log(1.8)
...
generated quantities {
  ...
  for (t in 1:N_trees) {
    ...
    + beta_ffp * (ffp_obs_tree - ffp0) + kappa_sh * f_sh[stand_idx, all_years_idxs_tree];
