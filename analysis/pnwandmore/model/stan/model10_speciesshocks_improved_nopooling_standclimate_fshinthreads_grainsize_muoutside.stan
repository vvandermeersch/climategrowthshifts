      ...
      for (i in 1:(end-start+1)) {
        ...
        profile("trees_loop") {
          for(t in stand_trees_start_idxs[s]:stand_trees_end_idxs[s]){
            ...
            vector[Ny] f = L_cov[sp][1:Ny, 1:Ny] * f_tilde[tree_start:tree_end];
            
            // if(s == 3 && ts == 5){
            //   print("Stand: ", s);
            //   print("Tree in stand: ", ts);
            //   print("Tree in data: ", t);
            //   print("Species: ", sp);
            //   print("Stand_species: ", stsp);
            //   print("Stand clim: ", stand_clim_idxs);
            //   print("Tree clim: ", stand_clim_idxs[all_years_idxs_tree]);
            // }
...
model {
  ...
  profile("compute_mu_outside2") {
    for (t in 1:N_trees) {
      int s  = stand_idxs[t];
      int sp = species_idxs[t];
  
      for (j in 1:N_years[t]) {
        int idx = tree_start_idxs[t] + j - 1;
        int ay  = all_years_idxs[idx];
        int clim_idx = (s - 1) * N_all_years + ay;
  
        mu[idx] =
            alpha
          + beta_gdd[sp] * (gdd_obs[clim_idx] - gdd0)
          + beta_pre[sp] * (pre_obs[clim_idx] - pre0)
          + beta_vpd[sp] * (vpd_obs[clim_idx] - vpd0)
          + kappa_sh[sp] * f_sh[s, ay];
      }
    }
  }
...
//         log_pshock = log(mw_shock) + normal_lpdf(log(rw_obs[tree_idxs[y]]) | mu[y] + f[tree_idxs[y]],
//           sqrt(tau_sck[species_idx]^2 + sigma^2));
//         log_pshock_plus_pnonshock = log_mix(mw_shock,
//           normal_lpdf(log(rw_obs[tree_idxs[y]])| mu[y] + f[tree_idxs[y]], sqrt(tau_sck[species_idx]^2 + sigma^2)),
//           normal_lpdf(log(rw_obs[tree_idxs[y]]) | mu[y] + f[tree_idxs[y]], sigma));
//       }else{
//         log_pshock = log(mw_shock) + normal_lcdf(log(epsilon) | mu[y] + f[tree_idxs[y]],
//           sqrt(tau_sck[species_idx]^2 + sigma^2));
//         log_pshock_plus_pnonshock = log_mix(mw_shock,
//           normal_lcdf(log(epsilon)| mu[y] + f[tree_idxs[y]], sqrt(tau_sck[species_idx]^2 + sigma^2)),
...
//         real log_y = normal_ub_rng(mu[y] + f[tree_idxs[y]], sqrt(tau_sck[species_idx]^2 + sigma^2), log(epsilon));
