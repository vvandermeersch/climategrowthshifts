      ...
      for (t in stand_tree_idxs[stand_trees_start_idxs[s]:stand_trees_end_idxs[s]]) {
        ...
        for (y in 1:N_years[t]) {
          ...
          if (rw_obs[idx] >= epsilon){
            ...
            lpd_nonconc[ys] += log(thetas_idio[t][1]) + lpds[1];
            lpd_conc[ys] += log_sum_exp(log(lambdas) + lpds);

          }else{
            
            vector[6] lpds = [
              normal_lcdf(log(epsilon) | mu_f, sigma), // no concordant shock, no idiosync. shutdown
            ...
            lpd_nonconc[ys] += log_sum_exp(
              log(thetas_idio[t][1]) + lpds[1], 
              log(thetas_idio[t][2]) + lpds[2]
            );
            lpd_conc[ys] += log_sum_exp(log(lambdas) + lpds);
          }
        }
...
parameters {
  ...
  array[N_trees] simplex[2] thetas_idio;
...
model {
  ...
  omega_shutdown ~ beta(1, 10); // 0% < omega_shutdown0 < 10%
  
  sigma ~ normal(0, log(1.5) / 2.57); // 0 < sigma < log(1.5)
  
  to_vector(f_tilde) ~ normal(0, 1);
  f_ind_tilde ~ normal(0, 1);
  
  // Idiosyncratic shocks
  vector[2] thetas_baseline = [100, 0.5]';
  real omega_thetas = 4;
  vector[2] ones = rep_vector(1, 2);
  vector[2] alphas = thetas_baseline / omega_thetas + ones;
