        ...
        for (y in 1:N_years[t]) {
          ...
          if (rw_obs[idx] >= epsilon){
            ...
            vector[6] lpds = [
              ...
              normal_lpdf(log_rw | mu_f, sqrt(tau_conc^2 + sigma^2)), // concordant depressed growth, no idiosync. shock
              normal_lpdf(log_rw | mu_f, sqrt(tau_idio_small^2 + tau_conc^2 + sigma^2)), // concordant depressed growth, small idiosync. depressed growth
              normal_lpdf(log_rw | mu_f, sqrt(tau_idio_large^2 + tau_conc^2 + sigma^2)) // concordant depressed growth, large idiosync. depressed growth
            ...
            vector[12] lpds = [
              ...
              normal_lcdf(log(epsilon) | mu_f, sqrt(tau_conc^2 + sigma^2)), // concordant depressed growth, no idiosync. shock
              normal_lcdf(log(epsilon) | mu_f, sqrt(tau_idio_small^2 + tau_conc^2 + sigma^2)), // concordant depressed growth, small idiosync. depressed growth
              normal_lcdf(log(epsilon) | mu_f, sqrt(tau_idio_large^2 + tau_conc^2 + sigma^2)), // concordant depressed growth, large idiosync. depressed growth
    ...
    for(y in 1:N_years[t]){
      ...
      if(sck_cat == 1 || sck_cat == 4){ // no concordant shock, no idiosyncratic shock
        ...
        if(rw_obs[idx] >= epsilon){
          ...
          // sample from a truncated normal distribution? between -inf and log(epsilon)
          log_rw = normal_ub_rng(mu_f, sqrt(tau_conc^2 + sigma^2), log(epsilon));
          residual = log_rw - mu_f;
          
        }
        
        // we can reconstruct shock posterior using the normal-normal conjugancy
        real conjugate_mean = (tau_conc^2 / (tau_conc^2 + sigma^2)) * residual;
        ...
        if(rw_obs[idx] >= epsilon){
          ...
          // sample from a truncated normal distribution? between -inf and log(epsilon)
          log_rw = normal_ub_rng(mu_f, sqrt(tau_idio_small^2 + tau_conc^2 + sigma^2), log(epsilon));
          residual = log_rw - mu_f;
          
        }
        
        // we can reconstruct shock posterior using the normal-normal conjugancy
        real conjugate_mean = ( (tau_idio_small^2 + tau_conc^2) / (tau_idio_small^2 + tau_conc^2 + sigma^2)) * residual;
        ...
        if(rw_obs[idx] >= epsilon){
          ...
          // sample from a truncated normal distribution? between -inf and log(epsilon)
          log_rw = normal_ub_rng(mu_f, sqrt(tau_idio_large^2 + tau_conc^2 + sigma^2), log(epsilon));
          residual = log_rw - mu_f;
          
        }
        
        // we can reconstruct shock posterior using the normal-normal conjugancy
        real conjugate_mean = ( (tau_idio_large^2 + tau_conc^2) / (tau_idio_large^2 + tau_conc^2 + sigma^2)) * residual;
      ...
      }else if(sck_cat == 12 || sck_cat >= 14){ // concordant shutdown (with a hidden idiosync. signal)
