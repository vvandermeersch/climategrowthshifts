        ...
        for (y in 1:N_years[t]) {
          ...
          if (rw_obs[idx] >= epsilon){
            ...
            vector[6] lpds = [
              ...
              normal_lpdf(log_rw | mu_f, sqrt(tau_small_idio^2 + sigma^2)), // no concordant shock, small idio. shock
              normal_lpdf(log_rw | mu_f, sqrt(tau_large_idio^2 + sigma^2)), // no concordant shock, large idio. shock
              
              normal_lpdf(log_rw | mu_f - mu_conc, sqrt(tau_conc^2 + sigma^2)), // concordant depressed growth, no idio. shutdown
              normal_lpdf(log_rw | mu_f - mu_conc, sqrt(tau_small_idio^2 + tau_conc^2 + sigma^2)), // concordant depressed growth, small idio. shock
              normal_lpdf(log_rw | mu_f - mu_conc, sqrt(tau_large_idio^2 + tau_conc^2 + sigma^2)) // concordant depressed growth, small idio. shock
            ]';
            ...
            lpd_nonconc[ys] += log_sum_exp(
              log([thetas_idio[t][1], thetas_idio[t][1], thetas_idio[t][3]]') +
            ...
            vector[12] lpds = [
              ...
              normal_lcdf(log(epsilon) | mu_f, sqrt(tau_small_idio^2 + sigma^2)), // no concordant shock, small idio. shock
              normal_lcdf(log(epsilon) | mu_f, sqrt(tau_large_idio^2 + sigma^2)), // no concordant shock, large idio. shock
              log(1), // no concordant shock, idiosync. shutdown
              
              normal_lcdf(log(epsilon)| mu_f - mu_conc, sqrt(tau_conc^2 + sigma^2)), // concordant depressed growth, no idio. shutdown
              normal_lcdf(log(epsilon) | mu_f - mu_conc, sqrt(tau_small_idio^2 + tau_conc^2 + sigma^2)), // concordant depressed growth, small idio. shock
              normal_lcdf(log(epsilon) | mu_f - mu_conc, sqrt(tau_large_idio^2 + tau_conc^2 + sigma^2)), // concordant depressed growth, small idio. shock
              log(1), // concordant depressed growth, idiosync. shutdown
              
              log(1), // concordant shutdown, no idiosync. shock
              log(1), // concordant shutdown, small idio. shock
              log(1), // concordant shutdown, large idio. shock
            ...
            lpd_nonconc[ys] += log_sum_exp(
              log([thetas_idio[t][1], thetas_idio[t][1], thetas_idio[t][3], thetas_idio[t][4]]') +
              [lpds[1], lpds[2], lpds[3], lpds[4]]'
            );
...
parameters {
  ...
  real<lower=0> tau_small_idio; // log variation scale
  real<lower=0> tau_large_idio; // log variation scale
...
model {
  ...
  mu_conc ~ normal(log(20), log(10)/2.57);
  ...
  gamma_sh ~ normal(0, log(1.5) / 2.57); // 0 < gamma_sh < log(1.5)
  
  rho_ind ~  lognormal(1.4, 0.35); // 2 < rho_ind < 11
  gamma_ind ~ normal(0, log(1.5) / 2.57); // 0 < gamma_sh < log(1.5)
  
  phi_sck ~ beta(2, 13); // 2% < phi_sck0 < 30% (30% is already a lot)
  
  omega_conc_sck ~ beta(2, 2); // 5% < omega_conc_sck0 < 40%
  
  omega_shutdown ~ beta(1, 5); // 0% < omega_shutdown0 < 10%
  
  sigma ~ normal(0, log(1.1) / 2.57); // 0 < sigma < log(1.5)
  
  to_vector(f_tilde) ~ normal(0, 1);
  f_ind_tilde ~ normal(0, 1);
  
  // Idiosyncratic shocks
  vector[4] thetas_baseline = [100, 10, 5, 0.5]';
  real omega_thetas = 4;
  vector[4] ones = rep_vector(1, 4);
  vector[4] alphas = thetas_baseline / omega_thetas + ones;
  
  tau_small_idio ~ normal(0, log(1.5) / 2.57);
  tau_small_idio ~ normal(0, log(5) / 2.57); // 0 < sigma < log(1.5)
  ...
  for (t in 1:N_trees) {
    ...
    for(y in 1:N_years[t]){
      ...
      if(rw_obs[idx] >= epsilon){
        ...
        vector[9] lambdas = [
          (1-phi_sck[stand_idx])*thetas_idio[t][1], 
          (1-phi_sck[stand_idx])*thetas_idio[t][2], 
          (1-phi_sck[stand_idx])*thetas_idio[t][3], 
        ...
        vector[9] lpds = [
          ...
          normal_lpdf(log_rw | mu_f, sqrt(tau_small_idio^2 + sigma^2)), // no concordant state, small idiosync. depressed growth
          normal_lpdf(log_rw | mu_f, sqrt(tau_large_idio^2 + sigma^2)), // no concordant state, large idiosync. depressed growth
      
          normal_lpdf(log_rw | mu_f, sigma), // no concordant shock, no idiosync. shock
          normal_lpdf(log_rw | mu_f, sqrt(tau_small_idio^2 + sigma^2)), // no concordant shock, small idiosync. depressed growth
          normal_lpdf(log_rw | mu_f, sqrt(tau_large_idio^2 + sigma^2)), // no concordant shock, large idiosync. depressed growth
              
          normal_lpdf(log_rw | mu_f - mu_conc, sqrt(tau_conc^2 + sigma^2)), // concordant depressed growth, no idiosync. shock
          normal_lpdf(log_rw | mu_f - mu_conc, sqrt(tau_small_idio^2 + tau_conc^2 + sigma^2)), // concordant depressed growth, small idiosync. depressed growth
          normal_lpdf(log_rw | mu_f - mu_conc, sqrt(tau_large_idio^2 + tau_conc^2 + sigma^2)) // concordant depressed growth, large idiosync. depressed growth
        ]'; 
        ...
        sck_cat = categorical_rng(jp);
         ...
         vector[16] lambdas = [
          ...
          (1-phi_sck[stand_idx])*thetas_idio[t][4], 
         ...
         vector[16] lpds = [
          ...
          normal_lcdf(log(epsilon) | mu_f, sqrt(tau_small_idio^2 + sigma^2)), // no concordant state, small idiosync. depressed growth
          normal_lcdf(log(epsilon) | mu_f, sqrt(tau_large_idio^2 + sigma^2)), // no concordant state, large idiosync. depressed growth
      
          normal_lcdf(log(epsilon) | mu_f, sigma), // no concordant shock, no idiosync. shock
          normal_lcdf(log(epsilon) | mu_f, sqrt(tau_small_idio^2 + sigma^2)), // no concordant shock, small idiosync. depressed growth
          normal_lcdf(log(epsilon) | mu_f, sqrt(tau_large_idio^2 + sigma^2)), // no concordant shock, large idiosync. depressed growth
              
          normal_lcdf(log(epsilon) | mu_f - mu_conc, sqrt(tau_conc^2 + sigma^2)), // concordant depressed growth, no idiosync. shock
          normal_lcdf(log(epsilon) | mu_f - mu_conc, sqrt(tau_small_idio^2 + tau_conc^2 + sigma^2)), // concordant depressed growth, small idiosync. depressed growth
          normal_lcdf(log(epsilon) | mu_f - mu_conc, sqrt(tau_large_idio^2 + tau_conc^2 + sigma^2)), // concordant depressed growth, large idiosync. depressed growth
      ...
      if(sck_cat == 1 || sck_cat == 4){ // no concordant shock, no idiosyncratic shock
        ...
        if(rw_obs[idx] >= epsilon){
          ...
          log_rw = normal_ub_rng(mu_f, sqrt(tau_small_idio^2 + sigma^2), log(epsilon));
          residual = log_rw - mu_f;
          
        }
        
        // we can reconstruct shock posterior using the normal-normal conjugancy
        real conjugate_mean = (tau_small_idio^2 / (tau_small_idio^2 + sigma^2)) * residual;
        real conjugate_sd   = sqrt((tau_small_idio^2 * sigma^2) / (tau_small_idio^2 + sigma^2));
        ...
        if(rw_obs[idx] >= epsilon){
          ...
          // sample from a truncated normal distribution? between -inf and log(epsilon)
          log_rw = normal_ub_rng(mu_f, sqrt(tau_large_idio^2 + sigma^2), log(epsilon));
          residual = log_rw - mu_f;
          
        }
        
        // we can reconstruct shock posterior using the normal-normal conjugancy
        real conjugate_mean = (tau_large_idio^2 / (tau_large_idio^2 + sigma^2)) * residual;
        real conjugate_sd   = sqrt((tau_large_idio^2 * sigma^2) / (tau_large_idio^2 + sigma^2));
        ...
        if(rw_obs[idx] >= epsilon){
          ...
          // sample from a truncated normal distribution? between -inf and log(epsilon)
          log_rw = normal_ub_rng(mu_f - mu_conc, sqrt(tau_small_idio^2 + tau_conc^2 + sigma^2), log(epsilon));
          residual = log_rw - mu_f;
          
        }
        
        // we can reconstruct shock posterior using the normal-normal conjugancy
        real conjugate_mean = - mu_conc + ((tau_small_idio^2 + tau_conc^2) / (tau_small_idio^2 + tau_conc^2 + sigma^2)) * (residual + mu_conc);
        real conjugate_sd   = sqrt(( (tau_small_idio^2  + tau_conc^2) * sigma^2) / (tau_small_idio^2 + tau_conc^2 + sigma^2));
        ...
        if(rw_obs[idx] >= epsilon){
          ...
          // sample from a truncated normal distribution? between -inf and log(epsilon)
          log_rw = normal_ub_rng(mu_f - mu_conc, sqrt(tau_large_idio^2 + tau_conc^2 + sigma^2), log(epsilon));
          residual = log_rw - mu_f;
          
        }
        
        // we can reconstruct shock posterior using the normal-normal conjugancy
        real conjugate_mean = - mu_conc + ((tau_large_idio^2 + tau_conc^2) / (tau_large_idio^2 + tau_conc^2 + sigma^2)) * (residual + mu_conc);
        real conjugate_sd   = sqrt(( (tau_large_idio^2  + tau_conc^2) * sigma^2) / (tau_large_idio^2 + tau_conc^2 + sigma^2));
