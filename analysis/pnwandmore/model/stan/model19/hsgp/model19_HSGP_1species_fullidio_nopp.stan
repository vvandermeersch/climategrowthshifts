      ...
      for (t in stand_tree_idxs[stand_trees_start_idxs[s]:stand_trees_end_idxs[s]]) {
        ...
        for (y in 1:N_years[t]) {
          ...
          if (rw_obs[idx] >= epsilon){
            ...
            lpd_nonconc[ys] += log_sum_exp(
              log([thetas_idio[t][1], thetas_idio[t][2], thetas_idio[t][3], thetas_idio[t][4]]') +
...
parameters {
  ...
  real<lower=tau_small_idio> tau_large_idio; // log variation scale
...
model {
  ...
  mu_conc ~ lognormal(0.154, 0.546); // log(1.5) to log(30)
  tau_conc ~ normal(0, 1.5 / 2.57); // 
  ...
  omega_conc_sck ~ beta(4.3, 1.15); // 40% < omega_conc_sck0 < 100%
  
  omega_shutdown ~ beta(1.15, 4.3); // 0% < omega_shutdown0 < 60%
  
  sigma ~ normal(0, log(1.3) / 2.57); // we expected log(1.1)
  ...
  tau_large_idio ~ normal(0, log(5) / 2.57); 
