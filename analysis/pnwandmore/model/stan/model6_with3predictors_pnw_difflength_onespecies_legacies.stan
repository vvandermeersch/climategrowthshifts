...
data {
  ...
  vector[N] vpd_obs_n1; // Observed VPD (JJMJJA) (hPa)
  vector[N] vpd_obs_n2; // Observed VPD (JJMJJA) (hPa)
  vector[N] vpd_obs_n3; // Observed VPD (JJMJJA) (hPa)
...
transformed parameters {
  ...
  for(i in 1:(Nleg+1)){
    beta_vpd_leg[i] = beta_vpd*exp(-(yleg[i])^2/(2*rho_beta_vpd^2));
  }
...
model {
  ...
  for (t in 1:N_trees) {
    ...
    + beta_vpd_leg[1] * (vpd_obs_tree - vpd0)
...
generated quantities {
  ...
  for (t in 1:N_trees) {
    ...
    vector[N_years[t]] mu1 = alpha + beta_gdd * (gdd_obs_tree - gdd0) + beta_sm * (sm_obs_tree - sm0)
    + beta_vpd * (vpd_obs_tree - sm0) + kappa * f_sh[all_years_idxs_tree];
