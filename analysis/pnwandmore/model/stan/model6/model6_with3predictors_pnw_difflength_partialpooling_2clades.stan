...
model {
  ...
  tau_gdd ~ normal(0, log(1.8) / 2.57); // variation of the order of 50%?
  tau_sm ~ normal(0, log(1.8) / 2.57); // variation of the order of 50%?
  tau_vpd ~ normal(0, log(1.8) / 2.57); // variation of the order of 50%?
  
  for (sp in 1:N_species) {
    beta_gdd_tilde[sp] ~ normal(0, 1);
    beta_sm_tilde[sp] ~ normal(0, 1);
    beta_vpd_tilde[sp] ~ normal(0, 1);
  }
...
generated quantities {
  ...
  for (t in 1:N_trees) {
    ...
    vector[N_years[t]] mu1 = alpha + beta_gdd[species_idx] * (gdd_obs_tree - gdd0) + beta_sm[species_idx] * (sm_obs_tree - sm0)
