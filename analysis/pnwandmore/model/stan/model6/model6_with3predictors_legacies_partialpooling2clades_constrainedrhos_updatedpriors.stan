...
data {
  ...
  vector[N] vpd_obs_n1; // Observed VPD, year n-1
  vector[N] vpd_obs_n2; // Observed VPD, year n-2
  vector[N] vpd_obs_n3; // Observed VPD, year n-3
...
parameters {
  ...
  vector<lower=0>[N_clades] mu_rho_vpd;
  vector<lower=0>[N_clades] tau_rho_vpd;
  vector<lower=0>[N_species] rho_vpd;
...
transformed parameters {
  ...
  array[N_species] vector[Nleg+1] beta_vpd_leg;
  for(sp in 1:N_species){
    for(i in 1:(Nleg+1)){
      beta_vpd_leg[sp][i] = beta_vpd[sp]*exp(-(yleg[i])^2/(2*rho_vpd[sp]^2));
    }
  }
...
model {
  ...
  mu_rho_vpd ~ normal(0, 2 / 2.57); // 0 <~ rho_beta_vpd <~ 2
  tau_rho_vpd ~ normal(0, 1 / 2.57); // max. variation of the order of 1 year?
  ...
  for (sp in 1:N_species) {
    ...
    rho_vpd[sp] ~ normal(mu_rho_vpd[clade_idxs[sp]], tau_rho_vpd[clade_idxs[sp]]);
  }
  ...
  for (t in 1:N_trees) {
    ...
    vector[N_years[t]] vpd_obs_tree_n1 = vpd_obs_n1[tree_idxs];
    vector[N_years[t]] vpd_obs_tree_n2 = vpd_obs_n2[tree_idxs];
    vector[N_years[t]] vpd_obs_tree_n3 = vpd_obs_n3[tree_idxs];
    
    vector[N_years[t]] mu =  alpha
    + beta_gdd[species_idx] * (gdd_obs_tree - gdd0)
    + beta_sm[species_idx] * (sm_obs_tree - sm0)
    + beta_vpd_leg[species_idx][1] * (vpd_obs_tree - vpd0)
    + beta_vpd_leg[species_idx][2] * (vpd_obs_tree_n1 - vpd0)
    + beta_vpd_leg[species_idx][3] * (vpd_obs_tree_n2 - vpd0)
    + beta_vpd_leg[species_idx][4] * (vpd_obs_tree_n3 - vpd0)
