functions {
  ...
  real loglikelihood_partial_sum(array[] int stand_ids_slice,
    ...
    for (i in 1:(end-start+1)) {
      ...
      for (st in 1:N_stand_species) {
        int start = 1+(st-1)*N_all_years;
        int end = st*N_all_years;
        int sp = stand_species_species_idxs[st];
        
        mu_stand_species[st] = alpha
        + beta_gdd[sp] * (gdd_obs[start:end] - gdd0)
        + beta_pre[sp] * (pre_obs[start:end] - pre0)
        + beta_vpd[sp] * (vpd_obs[start:end] - vpd0) 
        + kappa_sh[sp] * f_sh[s, 1:N_all_years];
      }
...
data {
  ...
  array[N_stand_species] int<lower=1, upper=N_stands> stand_species_stand_idxs;
  ...
  vector[N_stand_species*N_all_years] gdd_obs; // Observed gdd (all year) (x100 degC)
  vector[N_stand_species*N_all_years] pre_obs; // Observed winter precipitation (NDJFMA) (dm)
  vector[N_stand_species*N_all_years] vpd_obs; // Observed VPD (JJMJJA) (hPa)
