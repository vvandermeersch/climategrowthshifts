        ...
        profile("trees_loop") {
          for(t in stand_tree_idxs[stand_trees_start_idxs[s]:stand_trees_end_idxs[s]]){
            ...
            for(y in 1:N_years[t]) {
              ...
              log_prone += log_mix(omega_ind_sck[stsp],
                normal_lpdf(delta_ind[idx] | 0, tau_sck[sp]),
...
parameters {
  ...
  vector[N_stand_species] alpha_omega_ind_sck; // log-odds
...
transformed parameters {
  ...
  vector<lower=0, upper=1>[N_stand_species] omega_ind_sck = inv_logit(alpha_omega_ind_sck); // probabilities
}
