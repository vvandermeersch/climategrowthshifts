      ...
      for (i in 1:(end-start+1)) {
        ...
        profile("trees_loop") {
          for(t in stand_trees_start_idxs[s]:stand_trees_end_idxs[s]){
            ...
            profile("compute_f") {
              f = L_cov[sp][1:N_years[t], 1:N_years[t]] * f_tilde[tree_start:tree_end];
            }
            ...
            profile("store_trees") {
              for(y in 1:N_years[t]) {
                ...
                int y_id  = all_years_idxs[idx];
                int clim_idx = (s - 1) * N_all_years + y_id;
                int ys  = y_id - stand_start_years_idxs[s] + 1;
                
                real mu_f = alpha
                  + beta_gdd[sp] * (gdd_obs[clim_idx] - gdd0)
                  + beta_pre[sp] * (pre_obs[clim_idx] - pre0)
                  + beta_vpd[sp] * (vpd_obs[clim_idx] - vpd0)
                  + kappa_sh[sp] * f_sh[y_id]
                  + f[y];
