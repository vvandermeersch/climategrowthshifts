...
generated quantities {
  ...
  array[N] real log_rw_pred_noshock;
  vector[N] alpha_clim;
  vector[N] alpha_f;
  vector[N] f;
  vector[N] f_ind;
  ...
  vector[N_stands*N_all_years] num_idsc_sck_stdlvl = rep_vector(0,N_stands*N_all_years);
  vector[N_stands*N_all_years] num_dbl_sck_stdlvl = rep_vector(0,N_stands*N_all_years);
  vector[N_stands*N_all_years] num_sck_stdlvl = rep_vector(0,N_stands*N_all_years); // number of shocks, whatever the type
  ...
  for (t in 1:N_trees) {
    ...
    alpha_f[tree_start:tree_end] = alpha +  f[tree_start:tree_end];
    ...
    alpha_clim[tree_start:tree_end] = mu-alpha;
    
    // here I dropped the baselines..?
    vector[N_years[t]] clim;
    clim = 
    beta_gdd * (gdd_obs[tree_clim_idxs] - mean(gdd_obs[stand_clim_idxs]))
    + beta_pre * (pre_obs[tree_clim_idxs] - mean(pre_obs[stand_clim_idxs]))
    + beta_vpd * (vpd_obs[tree_clim_idxs] - mean(vpd_obs[stand_clim_idxs]));

    // mixture weight for shocks!
    real two_shocks = phi_sck[stand_idx]*omega_conc_sck[stand_idx]*pi_idsc_sck;
    real one_conc_shock = phi_sck[stand_idx]*omega_conc_sck[stand_idx]*(1-pi_idsc_sck);
    real one_idsc_shock = pi_idsc_sck * (phi_sck[stand_idx]*(1-omega_conc_sck[stand_idx]) + (1-phi_sck[stand_idx]));
    real zero_shock = (1-pi_idsc_sck) * (phi_sck[stand_idx]*(1-omega_conc_sck[stand_idx]) + (1-phi_sck[stand_idx]));
    ...
    for(y in 1:N_years[t]){
      ...
      if(rw_obs[idx] >= epsilon){
        ...
        lp_two_shocks = log(two_shocks) + normal_lpdf(log_rw | mu_f, sqrt(tau_sck^2 + tau_sck^2 + sigma^2));
          
        lp_one_conc_shock = log(one_conc_shock) + normal_lpdf(log_rw | mu_f, sqrt(tau_sck^2 + sigma^2));
          
        lp_one_idsc_shock = log(one_idsc_shock) + normal_lpdf(log_rw | mu_f, sqrt(tau_sck^2 + sigma^2));
        
        lp_zero_shock = log(zero_shock) + normal_lpdf(log_rw | mu_f, sigma);

      }else{
        
        lp_two_shocks = log(two_shocks) + normal_lcdf(log(epsilon) | mu_f, sqrt(tau_sck^2 + tau_sck^2 + sigma^2));
        
        lp_one_conc_shock = log(one_conc_shock) + normal_lcdf(log(epsilon) | mu_f, sqrt(tau_sck^2 + sigma^2));
          
        lp_one_idsc_shock = log(one_idsc_shock) + normal_lcdf(log(epsilon) | mu_f, sqrt(tau_sck^2 + sigma^2));
        
        lp_zero_shock = log(zero_shock) + normal_lcdf(log(epsilon) | mu_f, sigma);

      }
      
      lp_all = log_sum_exp([lp_two_shocks, lp_one_conc_shock, lp_one_idsc_shock, lp_zero_shock]');

      // probability of shock state
      real lambda_two_shocks = exp(lp_two_shocks - lp_all);
      real lambda_one_conc_shock = exp(lp_one_conc_shock - lp_all);
      real lambda_one_idsc_shock = exp(lp_one_idsc_shock - lp_all);
      real lambda_zero_shock = exp(lp_zero_shock - lp_all);
      
      vector[4] p = to_vector({lambda_two_shocks, lambda_one_conc_shock, lambda_one_idsc_shock, lambda_zero_shock});
      ...
      if(sck_cat == 4){ // no shocks
        log_rw_pred[idx] = normal_rng(mu_f, sigma);
      }else if(rw_obs[idx] >= epsilon){
        real log_rw = log(rw_obs[idx]);
        real residual = log_rw - mu_f;
      
        if(sck_cat == 1){ // double shocks!
        
          // we can reconstruct shock posterior using the normal-normal conjugancy
          real conjugate_mean = ((tau_sck^2 + tau_sck^2) / (tau_sck^2 + tau_sck^2 + sigma^2)) * residual;
          real conjugate_sd   = sqrt(((tau_sck^2 + tau_sck^2)  * sigma^2) / (tau_sck^2 + tau_sck^2 + sigma^2));
          
          delta_sck[idx] = normal_rng(conjugate_mean, conjugate_sd);
          log_rw_pred[idx] = normal_rng(mu_f + delta_sck[idx], sigma);
          
          num_dbl_sck_stdlvl[tree_clim_idxs[y]] += 1.0/N_stand_trees[stand_idx]; // shock counter
          num_sck_stdlvl[tree_clim_idxs[y]] += 1.0/N_stand_trees[stand_idx]; // shock counter
          ...
          if(sck_cat == 2){
            ...
            num_idsc_sck_stdlvl[tree_clim_idxs[y]] += 1.0/N_stand_trees[stand_idx]; // shock counter
          }
        ...
        if(sck_cat == 1){ // double shocks!
          ...
          real log_rw = normal_ub_rng(mu_f, sqrt(tau_sck^2 + tau_sck^2 + sigma^2), log(epsilon));
          real residual = log_rw - mu_f;
      ...
      log_rw_pred_noshock[idx] = normal_rng(mu_f, sigma);
    }
  }
}
