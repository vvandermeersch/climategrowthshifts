...
generated quantities {
  ...
  for (t in 1:N_trees) {
    ...
    f[tree_start:tree_end] = PHI_sp[all_years_idxs_tree, ] * (sqrt_spd_sp[sp] .* f_tilde[, t]); 
    ...
    // here I changed the baselines..?
    vector[N_years[t]] clim;
    clim = 
    beta_gdd[sp] * (gdd_obs[tree_clim_idxs] - mean(gdd_obs[stand_clim_idxs]))
    + beta_pre[sp] * (pre_obs[tree_clim_idxs] - mean(pre_obs[stand_clim_idxs]))
    + beta_vpd[sp] * (vpd_obs[tree_clim_idxs] - mean(vpd_obs[stand_clim_idxs]));

    // mixture weight for shocks!
    real one_conc_shock = phi_sck[stand_idx]*omega_conc_sck[st_sp];
    real zero_shock = phi_sck[stand_idx]*(1-omega_conc_sck[st_sp]) + (1-phi_sck[stand_idx]);
    ...
    for(y in 1:N_years[t]){
      ...
      if(rw_obs[idx] >= epsilon){
        ...
        lp_one_conc_shock = log(one_conc_shock) + normal_lpdf(log_rw | mu_f, sqrt(tau_sck[sp]^2 + sigma^2));
          
        lp_zero_shock = log(zero_shock) + normal_lpdf(log_rw | mu_f, sigma);

      }else{
        
        lp_one_conc_shock = log(one_conc_shock) + normal_lcdf(log(epsilon) | mu_f, sqrt(tau_sck[sp]^2 + sigma^2));
      ...
      if(sck_cat == 2){ // no shocks
        ...
        real conjugate_mean = (tau_sck[sp]^2 / (tau_sck[sp]^2 + sigma^2)) * residual;
        real conjugate_sd   = sqrt((tau_sck[sp]^2 * sigma^2) / (tau_sck[sp]^2 + sigma^2));
        ...
        real log_rw = normal_ub_rng(mu_f, sqrt(tau_sck[sp]^2 + sigma^2), log(epsilon));
        real residual = log_rw - mu_f;
