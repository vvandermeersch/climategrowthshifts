...
parameters {
  ...
  // GDD slopes (1/kdegC, partially pooled across species)
  real mu_gdd;
  real<lower=0> tau_gdd;
  vector[N_species] beta_gdd;
  
  // Precipitation slope (1/dm, partially pooled across species)
  real mu_pre;
  real<lower=0> tau_pre;
  vector[N_species] beta_pre;
  
  // VPD slope (1/hPa, partially pooled across species)
  real mu_vpd;
  real<lower=0> tau_vpd;
  vector[N_species] beta_vpd;
  
  // Short-term tree-level func. behavior(canopy dynamics)
  vector[N] f_ind_tilde; 
  real<lower=1> rho_ind; // length scale
  real<lower=0> gamma_ind; // marginal variation
  
  // Mid- to long-term tree-level func. behavior (allometry) -- approx. with HS
  matrix[M, N_trees] f_tilde;
  // length scales (partially pooled across species)
  real<lower=rho_ind> mu_rho; 
  real<lower=0> tau_rho;
  vector<lower=rho_ind>[N_species] rho_sp; 
  // marginal variations (partially pooled across species)
  real<lower=0> mu_gamma;
  real<lower=0> tau_gamma;
  vector<lower=0>[N_species] gamma_sp; 
  
  // Short-term stand-level proportional growth functional behavior
