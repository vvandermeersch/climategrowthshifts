...
writeLines(
  readLines(file.path(wd, 'model', 'stan', 
                      'model9_marginalshocks.stan'))[351:402])
```
...
I cannot do a proper retrodictive check because I need to finish the generated quantities block... But as a teaser, the latent shock states look nice (I think):
...
## Complete the generated quantities block

What we want is the posterior of $\delta_\text{shock}(k) ~|~ y(k)$, right?\
But I can't really see how I can recover the posterior without having ${\tau_\text{inner}^\text{shock}}$ or ${\tau_\text{outer}^\text{shock}}$. \
