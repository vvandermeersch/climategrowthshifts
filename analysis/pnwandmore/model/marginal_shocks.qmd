...
In terms of domain expertise, we have to rethink about our prior on $\tau_\text{outer}^\text{shock}$. Playing with this [magic tool](https://distribution-explorer.github.io/continuous/cauchy.html#cauchy), we could set a prior to have $0\lesssim\tau_\text{outer}^\text{shock}\lesssim1$, as it would imply rougly shocks between -12 and 12. So something like:
```{stan output.var = 'foo'}
#| eval: false
outer_tau_sck ~ normal(0, 1/ 2.57);
```
