...
target += normal_lpdf(delta_sck[n] | 0, outer_tau_sck)
```
Moving to a Cauchy, we would have this:
```{stan output.var = 'foo'}
#| eval: false
target += normal_lpdf(y[n] | mu[n] + delta_sck[n], sigma)
target += cauchy_lpdf(delta_sck[n] | 0, outer_tau_sck)
```
$\tau_\text{outer}^\text{shock}$ is now the scale of the Cauchy distribution:
$$\delta_\text{shock} \sim \text{Cauchy}(0, \tau_\text{outer}^\text{shock})$$
I came across Mike's [writing on Cauchy](https://betanalpha.github.io/assets/case_studies/fitting_the_cauchy.html#4_second_alternative_implementation), and in particular the second implementation. If I understood correctly, the Cauchy is a scale mixture of Gaussian density functions, and in particular we would have:
$$\delta_\text{shock} \mid \tau_\text{outer}^\text{aux} \sim \text{normal}(0, \tau_\text{outer}^\text{aux})$$
$${\tau_\text{outer}^\text{aux}}^2  \sim \text{InvGamma}(0.5, 0.5*{\tau_\text{outer}^\text{shock}}^2)$$
So, I think we could write:
```{stan output.var = 'foo'}
#| eval: false
target += normal_lpdf(y[n] | mu[n] + delta_sck[n], sigma)
target += normal_lpdf(delta_sck[n] | 0, sqrt(outer_tau2_aux))
target += inv_gamma_lpdf(outer_tau2_aux | 0.5, 0.5*outer_tau_sck^2)
```
And thus marginalize the two first lines and obtain:
```{stan output.var = 'foo'}
#| eval: false
target += normal_lpdf(y[n] | mu[n], sqrt(outer_tau2_aux+sigma^2))
target += inv_gamma_lpdf(outer_tau2_aux | 0.5, 0.5*outer_tau_sck^2)
```
Which may allow to keep the normal-normal conjugancy to reconstruct $\delta_\text{shock}$ afterwards..? I never thought I would be so much into conjugate distributions.\
In terms of domain expertise, we have to rethink about our prior on $\tau_\text{outer}^\text{shock}$. Playing with this [magic tool](https://distribution-explorer.github.io/continuous/cauchy.html#cauchy), we could set a prior to have $0\lesssim\tau_\text{outer}^\text{shock}\lesssim0.4$, as it would imply roughly shocks between -3 and 3 (Q5-Q95), and large shocks between -12 and 12 (Q1-Q99). So something like:
```{stan output.var = 'foo'}
#| eval: false
outer_tau_sck ~ normal(0, 0.4/ 2.57);
```
...
writeLines(
  readLines(file.path(wd, 'model', 'stan', 
                      'model9_marginalshocks_zerotauinner_cauchy.stan'))[281:318])
```
...
writeLines(
  readLines(file.path(wd, 'model', 'stan', 
                      'model9_marginalshocks_zerotauinner_cauchy.stan'))[357:416])
```
...
Before digging too much on what can make the model not to capture intermediate and large shocks together, I should first consider to better define what _is_ a large shock. In our two sites example, most large shocks actually correspond to missing rings---or at least below the measurement precision, which should be around $1\mathrm{e}{-3} ~\text{mm}$! And until now we were considering something like:
$$\log(y + \epsilon) ~ \text{with} ~ \epsilon = 1\mathrm{e}{-4} ~\text{mm}$$
Boooo! Loosers.\
...
writeLines(
  readLines(file.path(wd, 'model', 'stan', 
                      'model9_marginalshocks_zerotauinner_cauchy_censoring.stan'))[57:67])
```

Currently I just set: `real epsilon = 1e-3;`.

## Test on one shocky site
...
So, what parameters in this fit (orange) are different than the previous fit with only one stand (green)?
...
par(cex.lab=1.1, mfrow = c(2,2))
util$plot_expectand_pushforward(samples[['omega_conc_sck']]*samples[['phi_sck[1]']], 50,
                                flim = c(0,0.5), 
                                display_name=expression(phi[shock]^{stand~1} %*% omega[shock]^concordant),
                                col = '#81b29a')
util$plot_expectand_pushforward(samples2[['omega_conc_sck']]*samples2[['phi_sck[1]']], 50,
                                flim = c(0,0.5),
                                col = '#e07a5f', add = TRUE)

util$plot_expectand_pushforward( (samples[['omega_nonconc_sck']]*(1-samples[['phi_sck[1]']])), 50,
                                flim = c(0,0.5), 
                                display_name=expression((1-phi[shock]^{stand~1}) %*% omega[shock]^{non-concordant}),
                                col = '#81b29a')
util$plot_expectand_pushforward( (samples2[['omega_nonconc_sck']]*(1-samples2[['phi_sck[1]']])), 50,
                                flim = c(0,0.5),
                                col = '#e07a5f', add = TRUE)

util$plot_expectand_pushforward(samples2[['omega_conc_sck']]*samples2[['phi_sck[2]']], 50,
                                flim = c(0,0.5),
                                display_name=expression(phi[shock]^{stand~2} %*% omega[shock]^concordant),
                                col = '#e07a5f')

util$plot_expectand_pushforward( (samples2[['omega_nonconc_sck']]*(1-samples2[['phi_sck[2]']])), 50,
                                flim = c(0,0.5),
                                display_name=expression((1-phi[shock]^{stand~2}) %*% omega[shock]^{non-concordant}),
                                col = '#e07a5f')

```

In the second fit, I kind of feel that the difference between $\phi_\text{shock} * \omega_\text{shock}^\text{concordant}$ (Site 1, Q50 = `{r} round(util$ensemble_mcmc_quantile_est( (samples2[['omega_conc_sck']]*(samples2[['phi_sck[1]']])), c(0.5)), 2)`) and $(1-\phi_\text{shock}) * \omega_\text{shock}^\text{non-concordant}$ (Site 1, Q50 = `{r} round(util$ensemble_mcmc_quantile_est( (samples2[['omega_nonconc_sck']]*(1-samples2[['phi_sck[1]']])), c(0.5)), 2)`) is not as high as I would expect. In other words, at the tree level, the proportion of non-concordant shocks relative to concordant shocks is higher than I would expect. 
...
This would lead me to consider a distribution a bit less rigid.


<!-- # Only concordant shocks -->

<!-- Maybe starting with only concordant shocks, i.e. fixing $\omega_\text{shock}^\text{non-concordant}=0$, could help better understanding the model behavior. -->
...
<!-- if(FALSE){ -->
<!--   fit <- stan(file=file.path(wd, 'model', 'stan', 'model9_marginalshocks_zerotauinner_cauchy_censoring_onlyconcordant.stan'), -->
<!--             data=data, seed=5838293, chains = 4, -->
<!--             warmup=1000, iter=2024, refresh=10) -->
<!--   saveRDS(fit, file.path(wd, 'model/shocks/output/marginal', 'fit_cauchy1_onlyconcordant.rds')) -->
<!-- }else{ -->
<!--   fit <- readRDS(file.path(wd, 'model/shocks/output/marginal', 'fit_cauchy1_onlyconcordant.rds')) -->
<!-- } -->
...
<!-- samples_onlyconc <- util$extract_expectand_vals(fit) -->
<!-- base_samples <- util$filter_expectands(samples_onlyconc, -->
<!--                                        c('alpha',  -->
<!--                                          'beta_gdd', 'beta_sm', 'beta_vpd', -->
<!--                                          'rho_sh', 'gamma_sh', -->
<!--                                          'rho_sp', 'gamma_sp', -->
...
<!--   names <- sapply(idxs_t, -->
<!--                   function(x) paste0('log_rw_pred[', x, ']')) -->
<!--   util$plot_conn_pushforward_quantiles(samples_onlyconc, names, data$years[idxs_t], -->
<!--                                        xlab="", ylab='log_rw_pred', main = paste0('Tree ', data$uniq_tree_ids[t]), -->
<!--                                        display_ylim=c(-10, 2), display_xlim = c(1980, 2010)) -->
<!--   points(data$years[idxs_t], data$log_rw_obs[idxs_t], pch=16, cex=1, col="white") -->
<!--   points(data$years[idxs_t], data$log_rw_obs[idxs_t], pch=16, cex=0.6, col="black") -->
...
<!--                   function(x) paste0('sck_state[', x, ']')) -->
<!--   ticks <- c(unlist(lapply(seq(1980, 2000,10), function(x) c(x, rep(NA, 9)))), '2010') -->
<!--   util$plot_disc_pushforward_quantiles(samples_onlyconc, names, data$years[idxs_t], -->
<!--                                        xlab="", ylab='Shock state',  -->
<!--                                        display_ylim=c(-0.1, 1.1),  -->
<!--                                        xticklabs =  ticks) -->
<!--   abline(v =  c(2002,1996), lty = 3, col="darkgrey") -->
...
<!--   names <- sapply(1:data$N_all_years, -->
<!--                   function(x) paste0('f_sh[', data$stand_idxs[t],',',x, ']')) -->
<!--   util$plot_conn_pushforward_quantiles(samples_onlyconc, names, data$all_years, -->
<!--                                        xlab="", ylab='Stand GP',  -->
<!--                                        display_ylim=c(-2, 2), display_xlim = c(1980, 2010)) -->
<!--   abline(v = c(2002,1996), lty = 3, col="darkgrey") -->
<!-- } -->
...
<!-- Maybe we should not look at only 4 trees though. For example, if we compare `WMP05` with (left) and without (right) non-concordant shocks: -->
...
<!--                   function(x) paste0('log_rw_pred[', x, ']')) -->
<!--   util$plot_conn_pushforward_quantiles(samples, names, data$years[idxs_t], -->
<!--                                        xlab="", ylab='log_rw_pred', main = paste0('Tree ', data$uniq_tree_ids[t],', WITH non-concordant'), -->
<!--                                        display_ylim=c(-10, 2), display_xlim = c(1980, 2010)) -->
<!--   points(data$years[idxs_t], data$log_rw_obs[idxs_t], pch=16, cex=1, col="white") -->
<!--   points(data$years[idxs_t], data$log_rw_obs[idxs_t], pch=16, cex=0.6, col="black") -->
<!--   abline(v =  c(2002,1996), lty = 3, col="darkgrey") -->
...
<!--                   function(x) paste0('log_rw_pred[', x, ']')) -->
<!--   util$plot_conn_pushforward_quantiles(samples_onlyconc, names, data$years[idxs_t], -->
<!--                                        xlab="", ylab='log_rw_pred', main = paste0('Tree ', data$uniq_tree_ids[t],', WITHOUT non-concordant'), -->
<!--                                        display_ylim=c(-10, 2), display_xlim = c(1980, 2010)) -->
<!--   points(data$years[idxs_t], data$log_rw_obs[idxs_t], pch=16, cex=1, col="white") -->
<!--   points(data$years[idxs_t], data$log_rw_obs[idxs_t], pch=16, cex=0.6, col="black") -->
<!--   abline(v =  c(2002,1996), lty = 3, col="darkgrey") -->
...
<!-- I don't know whether we would _a priori_ really consider the several years after 2002 as shocks... \ -->
<!-- Do we see a difference then in the probability of shocks? Not really, so fixing  $\omega_\text{shock}^\text{non-concordant}=0$ does not really make a big difference here. -->
...
<!-- par(cex.lab=1.1, mfrow = c(1,1)) -->
<!-- util$plot_expectand_pushforward(samples[['omega_conc_sck']]*samples[['phi_sck[1]']], 50, -->
<!--                                 flim = c(0,0.5),  -->
<!--                                 display_name=expression(phi[shock]^{stand~1} %*% omega[shock]^concordant), -->
<!--                                 col = '#81b29a') -->
<!-- util$plot_expectand_pushforward(samples_onlyconc[['omega_conc_sck']]*samples_onlyconc[['phi_sck[1]']], 50, -->
<!--                                 flim = c(0,0.5), -->
<!--                                 col = '#e07a5f', add = TRUE) -->
...
<!--             data=data, seed=5838293, chains = 4, -->
<!--             warmup=1000, iter=2024, refresh=10) -->
<!--   saveRDS(fit, file.path(wd, 'model/shocks/output/marginal', 'fit_cauchy2_onlyconcordant.rds')) -->
<!-- }else{ -->
<!--   fit <- readRDS(file.path(wd, 'model/shocks/output/marginal', 'fit_cauchy2_onlyconcordant.rds')) -->
<!-- } -->

<!-- ``` -->

<!-- Interestingly we don't get the tree depth warnings anymore... -->
...
<!-- samples2_onlyconc <- util$extract_expectand_vals(fit) -->
<!-- base_samples <- util$filter_expectands(samples2_onlyconc, -->
<!--                                        c('alpha',  -->
<!--                                          'beta_gdd', 'beta_sm', 'beta_vpd', -->
<!--                                          'rho_sh', 'gamma_sh', -->
<!--                                          'rho_sp', 'gamma_sp', -->
...
<!-- But again, 2002 is not considered as a shock for the tree `WMP01` anymore... -->
...
<!--   names <- sapply(idxs_t, -->
<!--                   function(x) paste0('log_rw_pred[', x, ']')) -->
<!--   util$plot_conn_pushforward_quantiles(samples2_onlyconc, names, data$years[idxs_t], -->
<!--                                        xlab="", ylab='log_rw_pred', main = paste0('Tree ', data$uniq_tree_ids[t]), -->
<!--                                        display_ylim=c(-10, 2), display_xlim = c(1980, 2010)) -->
<!--   points(data$years[idxs_t], data$log_rw_obs[idxs_t], pch=16, cex=1, col="white") -->
<!--   points(data$years[idxs_t], data$log_rw_obs[idxs_t], pch=16, cex=0.6, col="black") -->
...
<!--                   function(x) paste0('sck_state[', x, ']')) -->
<!--   ticks <- c(unlist(lapply(seq(1980, 2000,10), function(x) c(x, rep(NA, 9)))), '2010') -->
<!--   util$plot_disc_pushforward_quantiles(samples2_onlyconc, names, data$years[idxs_t], -->
<!--                                        xlab="", ylab='Shock state',  -->
<!--                                        display_ylim=c(-0.1, 1.1),  -->
<!--                                        xticklabs =  ticks) -->
<!--   abline(v =  c(2002,1996), lty = 3, col="darkgrey") -->
...
<!--   names <- sapply(1:data$N_all_years, -->
<!--                   function(x) paste0('f_sh[', data$stand_idxs[t],',',x, ']')) -->
<!--   util$plot_conn_pushforward_quantiles(samples2_onlyconc, names, data$all_years, -->
<!--                                        xlab="", ylab='Stand GP',  -->
<!--                                        display_ylim=c(-2, 2), display_xlim = c(1980, 2010)) -->
<!--   abline(v = c(2002,1996), lty = 3, col="darkgrey") -->
<!-- } -->
...
<!-- In this stand, we would also expect 2002 to be considered as a shock... and again, it's not.\ -->
<!-- However, now many of the years are considered as slightly positive shocks, which is not great. -->
...
<!-- And the predictions for this site seem to systematically fall below the observations, a pattern we did not observe before (left, with $\omega_\text{shock}^\text{non-concordant}>0$). -->
...
<!-- I have some trouble to link the difference of shock behavior in stand 1 and stand 2 to the parameter values... In the predictions, stand 2 seems often shocky, but its $\phi_\text{shock}$ is not higher than in stand 1... -->
...
<!--          ncol=6, byrow=TRUE) -->
<!-- ) -->
<!-- util$plot_expectand_pushforward(samples2_onlyconc[['sigma']], 50, -->
<!--                                 flim = c(0,0.5),  -->
<!--                                 display_name=expression(sigma)) -->
<!-- xs <- seq(0, 2, 0.01) -->
<!-- ys <- dnorm(xs, 0, log(1.1) / 2.57) -->
...
<!-- util$plot_expectand_pushforward(samples2_onlyconc[['outer_tau_sck']], 50, -->
<!--                                 flim = c(0,1),  -->
<!--                                 display_name=expression(tau[shock]^outer)) -->
<!-- xs <- seq(0, 1, 0.01) -->
<!-- ys <- dnorm(xs, 0, 0.4 / 2.57) -->
...
<!-- util$plot_expectand_pushforward(samples2_onlyconc[['phi_sck[1]']], 50, -->
<!--                                 flim = c(0,1),  -->
<!--                                 display_name=expression(phi[schock])) -->
<!-- util$plot_expectand_pushforward(samples2_onlyconc[['phi_sck[2]']], 50, -->
<!--                                 flim = c(0,1),  -->
<!--                                 display_name=expression(phi[schock]), -->
<!--                                 add = T) -->
<!-- xs <- seq(0, 1, 0.01) -->
...
<!-- util$plot_expectand_pushforward(samples2_onlyconc[['omega_conc_sck']], 50, -->
<!--                                 flim = c(0.5,1),  -->
<!--                                 display_name=expression(omega[shock]^concordant)) -->
<!-- xs <- seq(0.5, 1, 0.01) -->
<!-- ys <- dbeta(xs, 230, 14) -->
