...
datasets$stand_species <- paste(datasets$grouped_stand, datasets$species_code, sep = '_')
...
stand_species_species_idxs <- c()
gdd_obs  <- c()
pre_obs <- c()
vpd_obs <- c()

for(s in uniq_stand_species_ids){
  
  sp <- species_idxs[stand_species_idxs == which(uniq_stand_species_ids == s)][1]
  stand_species_species_idxs <- c(stand_species_species_idxs, sp)
  
  dataset <- datasets[datasets$stand_species == s, 'dataset'][1]
...
data <- mget(c('N', 'N_all_years', 'N_trees', 
               ...
               'stand_species_idxs', 'stand_species_species_idxs', 'N_stand_species', 
