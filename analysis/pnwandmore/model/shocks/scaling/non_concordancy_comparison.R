...
param_samples <- readRDS(file.path(wd, 'output/model/scaling', 'param_samples_15jan2025_PIPO_standclimate_19802024_nonconcordant.rds'))
tree_samples <- readRDS(file.path(wd, 'output/model/scaling', 'tree_samples_15jan2025_PIPO_standclimate_19802024_nonconcordant.rds'))
...
old_param_samples <- fit_19802024$draws(variables = c("beta_gdd", "beta_vpd", "beta_pre", "phi_sck", "omega_conc_sck", "rho_sp", "rho_sh"))
...
util$ensemble_mcmc_quantile_est(t(matrix(old_param_samples[1:1000,1:4,paste0('phi_sck[',s,']')], nrow = 1000, ncol = 4)), c(0.5))
...
for(i in 255:256){
  ...
  samples <- lapply(1:dim(tree_samples)[3], function(k){t(matrix(tree_samples[1:dim(tree_samples)[1],1:dim(tree_samples)[2],k], nrow = dim(tree_samples)[1], ncol = dim(tree_samples)[2]))})
  names(samples) <- names
