...
output_pre1.5 <- foreach(v = vpds, .options.future = list(seed = TRUE)) %dofuture% {
...
p <- 7
output_pre7 <- foreach(v = vpds, .options.future = list(seed = TRUE)) %dofuture% {
...
phis_pre1.5 <- sapply(output_pre1.5, function(l){l[["phi"]]})
rws_pre1.5 <- sapply(output_pre1.5, function(l){l[["rw"]]})
logrws_pre1.5 <- sapply(output_pre1.5, function(l){l[["logrw"]]})
mus_pre1.5 <- sapply(output_pre1.5, function(l){l[["mu"]]})
expmus_pre1.5 <- sapply(output_pre1.5, function(l){l[["expmu"]]})

phis_pre7 <- sapply(output_pre7, function(l){l[["phi"]]})
rws_pre7 <- sapply(output_pre7, function(l){l[["rw"]]})
logrws_pre7 <- sapply(output_pre7, function(l){l[["logrw"]]})
mus_pre7 <- sapply(output_pre7, function(l){l[["mu"]]})
expmus_pre7 <- sapply(output_pre7, function(l){l[["expmu"]]})

par(mfrow=c(1,2))
plot(y = NULL, x = NULL,
     xlab = "Summer VPD (hPa)", ylab = "log(ring width)",
     bty = "n",
     xlim = c(8, 36), ylim = c(-2, 0.5))

polygon(c(vpds, rev(vpds)),
        c(logrws_pre7["5%", ], rev(logrws_pre7["95%", ])),
        col = "#2f667d33", border = NA)

polygon(c(vpds, rev(vpds)),
        c(logrws_pre1.5["5%", ], rev(logrws_pre1.5["95%", ])),
        col = "#C75B3933", border = NA)


# Expected growth
lines(vpds, logrws_pre7["95%", ], col = "#2f667d", lty = 2, lwd = 1.2)
lines(vpds, logrws_pre7["5%",  ], col = "#2f667d", lty = 2, lwd = 1.2)
lines(vpds, logrws_pre7["50%", ], col = "#2f667d", lwd = 2.5)

# Ring width
lines(vpds, logrws_pre1.5["95%", ], col = "#C75B39", lty = 2, lwd = 1.2)
lines(vpds, logrws_pre1.5["5%",  ], col = "#C75B39", lty = 2, lwd = 1.2)
lines(vpds, logrws_pre1.5["50%", ], col = "#C75B39", lwd = 2.5)
...
polygon(c(vpds, rev(vpds)),
        c(phis_pre7["5%", ], rev(phis_pre7["95%", ])),
        col = "#2f667d33", border = NA)

polygon(c(vpds, rev(vpds)),
        c(phis_pre1.5["5%", ], rev(phis_pre1.5["95%", ])),
        col = "#C75B3933", border = NA)

lines(vpds, phis_pre7["95%", ], col = "#2f667d", lty = 2, lwd = 1.2)
lines(vpds, phis_pre7["5%",  ], col = "#2f667d", lty = 2, lwd = 1.2)
lines(vpds, phis_pre7["50%", ], col = "#2f667d", lwd = 2.5)

lines(vpds, phis_pre1.5["95%", ], col = "#C75B39", lty = 2, lwd = 1.2)
lines(vpds, phis_pre1.5["5%",  ], col = "#C75B39", lty = 2, lwd = 1.2)
lines(vpds, phis_pre1.5["50%", ], col = "#C75B39", lwd = 2.5)

legend(
  x = 8, y = 0.95,
  legend = c('Dry winter (150 mm)', 'Wet winter (700 mm)'),
  col = c("#C75B39", "#2f667d"),
  pch = 15,
  pt.cex = 1.3,
  cex = 0.8,
