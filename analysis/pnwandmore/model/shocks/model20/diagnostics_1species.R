...
base_samples <- readRDS(file.path(folder, 'model20', 'basesamples_model20_HSGP_PIPO_1stand_co654_19502008_shiftedsmall.rds'))
util$check_all_expectand_diagnostics(base_samples)

util$plot_pairs_by_chain(base_samples[['tau_sck']], 'tau_sck',
                         base_samples[['mu_sck']], 'mu_sck')

util$plot_pairs_by_chain(base_samples[['mu_small']], 'mu_small',
                         base_samples[['tau_small']], 'tau_small')
...
util$plot_expectand_pushforward(base_samples[['mu_small']], 30,
                                display_name = 'mu_small',
                                flim = c(0,1))
...
util$plot_expectand_pushforward(base_samples[['thetas_conc[1,1]']], 30,
                                display_name = 'p(no tree response | stand concord.)',
                                flim = c(0,1))
...
util$plot_expectand_pushforward(base_samples[['thetas_conc[1,2]']], 30,
                                display_name = 'p(small response | stand concord.)',
                                flim = c(0,1))
...
util$plot_expectand_pushforward(base_samples[['thetas_conc[1,3]']], 30,
                                display_name = 'p(extreme response | stand concord.)',
                                flim = c(0,1))
...
util$plot_expectand_pushforward(base_samples[['omega_shutdown[1]']], 30,
                                display_name = 'p(shutdown | extreme response)',
                                flim = c(0,1))
