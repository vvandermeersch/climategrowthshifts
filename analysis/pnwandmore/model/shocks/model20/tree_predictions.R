...
fit <- readRDS(file.path(folder, 'model20', 'fit_model20_HSGP_1species_shiftedsmall.rds'))
...
treesamples <- readRDS(file.path(folder, 'model20', 'treesamples_model20_HSGP_PIPO_1stand_co654_19502008_shiftedsmall.rds'))
...
util$plot_conn_pushforward_quantiles(treesamples, names, data$years[idxs],
                                     ...
                                     display_ylim=c(-3, 1), display_xlim = range(data$all_years))
...
names <- paste0("small[",idxs,"]")
util$plot_disc_pushforward_quantiles(treesamples, names, 
                                     xlab="Year", ylab="Small concordant state", 
                                     display_ylim=c(0, 1))

names <- paste0("large[",idxs,"]")
util$plot_disc_pushforward_quantiles(treesamples, names, 
                                     xlab="Year", ylab="Large concordant response", 
                                     display_ylim=c(0, 1))
