...
fit <- readRDS(file.path(folder, 'model17/fit_HSGP_26mar2026_WestCoast_19202024.rds'))
gc()
...
util$plot_disc_pushforward_quantiles(samples, paste0('kappa_sh[',1:data$N_species,']'),
                                     ylab = 'kappa_sh', display_ylim = c(0, 5))
...
util$plot_disc_pushforward_quantiles(samples, paste0('tau_sck[',1:data$N_species,']'),
                                     ylab = 'tau_sck', display_ylim = c(0, 3))


shocks <- fit$draws(c('phi_sck', 'omega_conc_sck', 'pi_idsc_sck'))
