...
data <- readRDS(file.path(folder,'data_06apr2026_PSME_colorado_19202024.rds'))
fit <- readRDS(file.path(folder, 'model17/fit_model17_HSGP_PSME_colorado_pi0.rds'))
gc()
...
names(samples) <- names
# samples <- readRDS(file.path(folder, 'model17/base_samples_model17_HSGP_PSMEexcept2_pi0.rds'))
...
lines(density(prior), col = util$c_light_teal, lwd = 1.5, lty = 2)
util$plot_expectand_pushforward(samples[['gamma_sp']], 50, flim= c(0,1), 'gamma_sp')
...
pdf(file.path(wd, 'model/shocks/model17/figures', 'PSME_coloarado_without_idiosyncracy_propshocks.pdf'), height = 13, width = 10)
