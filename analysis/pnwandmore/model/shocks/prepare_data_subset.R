...
table(datasets$species_code)
datasets <- datasets[datasets$species_code == 'PIPO' & datasets$state == 'az' & datasets$dataset == 'az618',]
...
for(d in 1:nrow(datasets)){
  ...
  raw_data_d <- raw_data_d[raw_data_d$year >= 1902 & raw_data_d$year <= 2001 & !is.na(raw_data_d$year), ]
...
saveRDS(data, file = file.path(wd, 'output/model/climatena', 'data_15oct2025_long_az618_before2002.rds'))
saveRDS(datasets, file.path(wd, 'output/model/climatena', 'datasets_15oct2025_long_az618_before2002.rds'))
