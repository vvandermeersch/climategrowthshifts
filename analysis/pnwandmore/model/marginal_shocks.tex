...
\FunctionTok{writeLines}\NormalTok{(}
  \FunctionTok{readLines}\NormalTok{(}\FunctionTok{file.path}\NormalTok{(wd, }\StringTok{\textquotesingle{}model\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}stan\textquotesingle{}}\NormalTok{, }
                      \StringTok{\textquotesingle{}model9\_marginalshocks.stan\textquotesingle{}}\NormalTok{))[}\DecValTok{351}\SpecialCharTok{:}\DecValTok{402}\NormalTok{])}
\end{Highlighting}
\end{Shaded}
...
I cannot do a proper retrodictive check because I need to finish the
generated quantities block\ldots{} But as a teaser, the latent shock
states look nice (I think):
...
\ControlFlowTok{for}\NormalTok{(t }\ControlFlowTok{in} \FunctionTok{sample}\NormalTok{(}\DecValTok{1}\SpecialCharTok{:}\NormalTok{data}\SpecialCharTok{$}\NormalTok{N\_trees, }\DecValTok{4}\NormalTok{))\{}
...
\subsection{Complete the generated quantities
block}\label{complete-the-generated-quantities-block}

What we want is the posterior of \(\delta_\text{shock}(k) ~|~ y(k)\),
right?\\
But I can't really see how I can recover the posterior without having
