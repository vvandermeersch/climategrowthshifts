...
base_samples <- readRDS(file.path(wd, "output/model/samples_24sept2025_partialpooling2clades_1interaction.rds"))
...
betas_climate <- data.frame(
  ...
  betasm.vpd_q5 = sapply(1:data$N_species, function(sp)  util$ensemble_mcmc_quantile_est(base_samples[[paste0('beta_sm_vpd[', sp, ']')]], 0.05)),
  betasm.vpd_q50 = sapply(1:data$N_species, function(sp)  util$ensemble_mcmc_quantile_est(base_samples[[paste0('beta_sm_vpd[', sp, ']')]], 0.5)),
  betasm.vpd_q95= sapply(1:data$N_species, function(sp)  util$ensemble_mcmc_quantile_est(base_samples[[paste0('beta_sm_vpd[', sp, ']')]], 0.95))
)
...
ggsave(comb, file = file.path(wd, 'figures', 'newyork2025', 'model_estimates', 'winteraction', 'climatesensitivity_subclades.pdf'),
       height = 200, width = 400, units = 'mm')


# Interaction terù
gdd_int_grid <- ggplot(data = betas_climate) +
  facet_grid(~subclade) +
  geom_vline(aes(xintercept = 0), linetype = 'dotted', color = 'grey70') + 
  geom_hline(aes(yintercept = 0), linetype = 'dotted', color = 'grey70') + 
  geom_pointrange(aes(x=betasm.vpd_q50,
                      ymin=betagdd_q5, y=betagdd_q50, ymax=betagdd_q95,
                      color = subclade), size = 0.2) +
  geom_pointrange(aes(xmin=betasm.vpd_q5, x=betasm.vpd_q50, xmax=betasm.vpd_q95,
                      y=betagdd_q50,
                      color = subclade), size = 0.2) +
  scale_color_manual(
    breaks = c('Pinus', 'Picea', 'Laricoideae', 'Abietoideae', 'Cupressaceae',
  ...
  theme(legend.position = 'none',
        strip.background = element_blank(),
        panel.grid = element_blank()) +
  labs(x = "SM x VPD", y = "GDD")

sm_int_grid <- ggplot(data = betas_climate) +
  facet_grid(~subclade) +
  geom_vline(aes(xintercept = 0), linetype = 'dotted', color = 'grey70') + 
  geom_hline(aes(yintercept = 0), linetype = 'dotted', color = 'grey70') + 
  geom_pointrange(aes(x=betasm.vpd_q50,
  ...
  theme(legend.position = 'none',
        strip.background = element_blank(),
        panel.grid = element_blank()) +
  labs(x = "SM x VPD", y = "Soil moisture")

vpd_int_grid <- ggplot(data = betas_climate) +
  facet_grid(~subclade) +
  geom_vline(aes(xintercept = 0), linetype = 'dotted', color = 'grey70') + 
  geom_hline(aes(yintercept = 0), linetype = 'dotted', color = 'grey70') + 
  geom_pointrange(aes(x=betasm.vpd_q50,
  ...
  theme(legend.position = 'none',
        strip.background = element_blank(),
        panel.grid = element_blank()) +
  labs(x = "SM x VPD", y = "VPD")

comb <- gdd_int_grid + sm_int_grid + vpd_int_grid + plot_layout(ncol = 1)
ggsave(comb, file = file.path(wd, 'figures', 'newyork2025', 'model_estimates', 'winteraction', 'interactionterm_climatesensitivity_subclades.pdf'),
       height = 200, width = 400, units = 'mm')
