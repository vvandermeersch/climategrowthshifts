...
parameters {
  ...
  // VPDxSM interaction (1/(hPA*%) ?)
  real beta_smvpd; // root value
...
model {
  ...
  beta_smvpd ~ normal(0, log(1.8) / 2.57); // -log(1.8) <~ beta_gsl <~ log(1.8)
  ...
  for (t in 1:N_trees) {
    ...
    + beta_smvpd * (sm_obs_tree - sm0) .* (vpd_obs_tree - vpd0)
...
generated quantities {
  ...
  for (t in 1:N_trees) {
    ...
    + beta_vpd * (vpd_obs_tree - sm0) + beta_smvpd * (vpd_obs_tree - sm0) .* (sm_obs_tree - sm0) + kappa_sh[species_idx] * f_sh[stand_idx, all_years_idxs_tree];
