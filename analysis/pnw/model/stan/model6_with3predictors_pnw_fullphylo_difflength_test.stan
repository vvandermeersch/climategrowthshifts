...
model {
  ...
  matrix[N_all_years, N_all_years] cov =  gp_exp_quad_cov(all_years, gamma, rho) 
  ...
  for (t in 1:N_trees) {
    ...
    //   =  gp_exp_quad_cov(years_tree, gamma, rho)
...
generated quantities {
  ...
  for (t in 1:N_trees) {
    ...
    + beta_vpd * (vpd_obs_tree - sm0) + f_sh[stand_idx, all_years_idxs_tree];
    
    vector[N_years[t]] mu2 = gp_pred_rng(years_tree, log_rw_obs[tree_idxs], years_tree, 
                                 mu1, gamma, rho, sigma, 1e-10) - mu1;             
