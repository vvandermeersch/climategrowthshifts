...
# fit <- readRDS(file.path(wd, 'output/model', 'fit_28june2025_completepooling.rds')) # run on Margot
...
# saveRDS(base_samples, file.path(wd, 'output/model', 'logrwpred_28june2025_completepooling.rds'))
base_samples <- readRDS(file.path(wd, 'output/model', 'logrwpred_28june2025_completepooling.rds')) 


pdf(file = file.path(wd, 'figures', 'model28june', 'conditional_medians.pdf'), width = 8.27, height = 11.69)
...
pdf(file = file.path(wd, 'figures', 'model28june', 'conditional_medians_residuals.pdf'), width = 8.27, height = 11.69)
par(mfrow = c(3,1), mar = c(4,4,1,1))
pred_names <- sapply(1:data$N, function(n) paste0('log_rw_pred[', n, ']'))
util$plot_conditional_median_quantiles(base_samples, pred_names, data$gdd_obs,
                                       2, 51, 1, data$log_rw_obs, 
