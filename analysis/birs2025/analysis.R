...
raw_data1 <- read.csv('data/abam_allplots.csv')
...
raw_data1$rw_core_ave..mm. <- as.vector(by(raw_data1, 
                                           seq_len(nrow(raw_data1)), 
                                           function(r) robust_ave(r[[5]], r[[6]])))

uniq_tree_ids <- unique(raw_data1$tree_id)
N_trees <- length(uniq_tree_ids)

N_stands <- length(unique(raw_data1$stand))

# Plot ring width data
par(mfrow=c(1, 1))

tidx <- 50
tid <- uniq_tree_ids[tidx]
rw_ave <- raw_data1$rw_core_ave..mm.[raw_data1$tree_id == tid]
util$plot_line_hist(rw_ave, 0, 3.5, 0.1,
                    xlab="Ring Width (mm)", 
                    main=paste("Core Average, Tree", tid))

par(mfrow=c(1, 1))

year <- raw_data1$year[raw_data1$tree_id == tid]
rw_ave <- raw_data1$rw_core_ave..mm.[raw_data1$tree_id == tid]
...
uniq_stand_ids <- unique(raw_data1$stand)
N_stands <- length(unique(raw_data1$stand))

# Common years
all_years <- NULL
for (tid in uniq_tree_ids) {
  years <- raw_data1[raw_data1$tree_id == tid,]$year
...
for (tid in uniq_tree_ids) {
  raw_data1_tree <- raw_data1[raw_data1$tree_id == tid & raw_data1$year %in% all_years,]
  
  years_tree <- raw_data1_tree$year
  all_years_idxs_tree <- sapply(years_tree, function(y) which(all_years == y))
  N_years_tree <- length(years_tree)
  ...
  stand_tree <- which(uniq_stand_ids == raw_data1_tree$stand[1])
...
for (s in 1:data$N_stands) {
  rw_names <- sapply(1:data$N_all_years,
                     function(n) paste0('f_sh[', s, ',', n, ']'))
  util$plot_realizations(samples, rw_names, data$all_years,
                         xlab="Year", ylab="Ring Width (mm)", 
                         display_ylim=c(-1, 1),
                         main=paste("Stand", uniq_stand_ids[s]))
  abline(v=1974, lwd=2, lty=2, col="#DDDDDD")
}
...
raw_data1 <- read.csv('data/allspecies_allplots.csv')
...
uniq_species_ids <- unique(raw_data1$species) #  Important that Abam is first here!
...
for (tid in uniq_tree_ids) {
  ...
  species_tree <- which(uniq_species_ids == raw_data1_tree$species[1])
...
for (t in 60*(1:10)) {
  ...
  plot(year, rw_ave, pch=16, cex=1.0, 
       xlab="Year", ylab="Ring Width Per mm", ylim=c(0, 2),
       main=paste("Stand", uniq_stand_ids[stand_idxs[t]],
...
# Posterior Quantification
fit <- stan(file='stan_programs/model6.stan',
            data=data, seed=5838299,
            warmup=1000, iter=2024, refresh=10)
